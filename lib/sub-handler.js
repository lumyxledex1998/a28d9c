...
const SPAM_CFG = {
  ...
  outbound: {
    windowMs: 60_000,
    maxPerChatPerMinGroup: 12,
    maxPerChatPerMinDM: 4,
    minReplyDelayMs: 250,
    maxReplyDelayMs: 900,
    maxTextLengthGroup: 700,
    maxMentionsGroup: 10,
    maxGlobalPerMin: 20
  }
}
...
function ensureAntiSpamState(conn) {
  ...
  conn.__sbAnti = { processedIds: new Map(), inbound: new Map(), outPerChat: new Map(), outGlobal: [] }
  if (typeof conn.sendMessage === 'function' && !conn._sbSendWrapped) {
    ...
    conn.sendMessage = async (jid, content, options = {}) => {
      try {
        ...
        // Global cap across all chats
        conn.__sbAnti.outGlobal = (conn.__sbAnti.outGlobal || []).filter(t => now - t < SPAM_CFG.outbound.windowMs)
        if (conn.__sbAnti.outGlobal.length >= SPAM_CFG.outbound.maxGlobalPerMin) {
          return
        }
        if (isGroup && content) {
        ...
        conn.__sbAnti.outGlobal.push(now)
        return res
...
export async function handler(chatUpdate) {
  ...
  try {
    m = smsg(conn, m)
    if (!m) return
    // Skip processing if just connected and under quiet period
    try { if (conn.__quietUntil && Date.now() < conn.__quietUntil) return } catch {}
