...
return conn.reply('🍓 *Se actualizo el pack y autor por defecto para tus stickers*', ctxErr) 
}
if (command === 'delmeta') {
if (!global.db.data.users[userId] || (!global.db.data.users[userId].text1 && !global.db.data.users[userId].text2)) {
return conn.reply('🌟 *No tienes establecido un pack de stickers', ctxErr)
}
const packstickers = global.db.data.users[userId]
delete packstickers.text1
delete packstickers.text2
await global.db.write()
return conn.reply('🍓 *Se restablecio el pack y autor por defecto para tus stickers', ctxErr)
}}
